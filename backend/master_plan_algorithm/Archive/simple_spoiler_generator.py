...
from aam_data_structure.geometry_utils.base_utils import create_enumerable_list
...
class SimpleSpoilerGenerator:
    ...
    # def _generate_brep(self, file_layer_geometry: list, height: float):
    #     """
    #     create brep geometries, not used
    #     """
    #     model_list = []
    #     test_element = PartElement()
    #     for geo in file_layer_geometry:
    #         brepGeo = create_brep_by_boundary(geo)
    #         brepFaceGeo = brepGeo.Faces[0]
    #         brep_model = extrude_brep(brepFaceGeo, height, True)
    #         model_list.append(brep_model)
    #     return model_list
    #
    # def _perform_union(self, breps):
    #     """
    #     perform boolean union, not used
    #     """
    #     union = Rhino.Geometry.Brep.CreateBooleanUnion(breps, 0.01)
    #     return union
    #
    # def _union_two(self):
    #     """
    #     function for testing boolean union, not used
    #     """
    #     union_element = PartElement()
    #     outline_model = self._generate_brep(self.outline.geometry, 3)
    #     buildings_model = self._generate_brep(self.buildings.geometry, self.params['building_height'])
    #     brep_list = outline_model+buildings_model
    #
    #     final_list = create_brep_list(brep_list)
    #
    #     things = self._perform_union(final_list)
    #     union_model = self._perform_union(final_list)[0]
    #     for brep in things:
    #         union_element.geometry.append(brep)
    ...
    def _operate_brep(self):
        ...
        """
        union_element = PartElement()
        self.outline_brep = self._generate_brep(self.outline.geometry, -2.0)
        self.buildings_brep = self._generate_brep(self.buildings.geometry, self.params['building_height'])
        self.river_bank_brep = self._generate_brep(self.river_bank.geometry, 1.2)
        self.grass_good_brep = self._generate_brep(self.grass_good.geometry, 2.2,)
        self.grass_better_brep = self._generate_brep(self.grass_better.geometry, 2.2,)
        brep_list = self.river_bank_brep + self.buildings_brep+ self.outline_brep
        final_list = create_brep_list(brep_list)
        things = self._perform_union(final_list)
        union_model = self._perform_union(final_list)[0]
        for brep in things:
            union_element.geometry.append(brep)
        union_element.object_name= 'union'
        return union_element
